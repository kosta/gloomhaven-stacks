...
export class PartyBattleGoals extends React.Component<NoProps, PartyBattleGoalsState> {
  ...
  componentDidMount() {
    ...
    try {
      ...
      const picks = new Map()
      serializedPicks
        .map((it) => it.split(PartyBattleGoals.pickStorageSeparator))
        .forEach((it) => {
    ...
    return (
      <div key="player-selection-container" style={playerSelectionStyle}>
        {range(0, 4).map((player) => {
          ...
          return (
            <button key={`select-player-${player}`} style={style} onClick={() => this.handlePlayerToggle(player)}>
              Player {player + 1}
            </button>
          )
        })}
  ...
  private battleGoalPicker(battleGoalsPerPlayer: Array<Array<BattleGoal>>) {
    ...
    return playerBattleGoals.map((battleGoal, index) => {
      ...
      const playerPickedCurrentBattleGoal =
        pickedBattleGoalId !== undefined && pickedBattleGoalId.equals(battleGoal.globalCardId)
      const notPickedCurrentBattleGoal = !playerPickedCurrentBattleGoal
      return (
        <div
          key={`battle-goal-${index}`}
          style={style}
        ...
        >
          <BattleGoalCard
            key={battleGoal.globalCardId.toString()}
  ...
  private dealer() {
    ...
    return (
      <div style={style} key="pool-configurator">
        ...
        <input type="checkbox" checked={this.state.includeSatireGaming} onChange={this.toggleIncludeSatireGaming} />
        <label>
          <a href="http://eepurl.com/dEDLkH" target="_blank">
            Satire Gaming
          </a>
        </label>
        <button disabled={!readyToDrawCards} onClick={this.handleDrawBattleGoals}>
          draw
        </button>
      </div>
    )
